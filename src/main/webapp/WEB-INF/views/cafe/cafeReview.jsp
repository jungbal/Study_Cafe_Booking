...
<div class="cafeReview">
	...
	<c:if test="${hasHistory}">
		<div class="comment-form">
			<form action="/cafeDetail/review/updateComment" method="post">
				<input type="hidden" name="cafeNo" value="${cafe.cafeNo}" /> <input
					type="hidden" name="writerId" value="${loginCafe.loginId}" />
	...
	<c:if test="${not hasHistory}">
		<div class="comment-form">
			<p style="color: gray; font-style: italic;">※ 이용내역이 있는 이용자만 리뷰 작성이 가능합니다.</p>
		</div>
	</c:if>
		...
		<c:forEach var="review" items="${reviewList}">
			<c:if test="${empty review.commentParent}">
				<div class="comment" style="margin-bottom: 20px;">
					...
					<p>
						<strong>${review.commentUserId}</strong> (${review.commentTime})
					</p>
					<p>${review.content}</p>

					<!-- 삭제 버튼: 관리자만 가능 -->
					<c:if test="${not empty loginCafe and loginCafe.loginRole == 1}">
						<form action="/cafeDetail/review/deleteComment" method="post"
					...
					<c:if
						test="${not empty loginCafe and loginCafe.loginId eq cafe.hostId}">
						<button type="button"
							onclick="toggleReplyForm('${review.commentId}')">답글 달기</button>

						<div class="reply-form" id="reply-form-${review.commentId}"
					...
					<c:forEach var="reply" items="${reviewList}">
						<c:if
							test="${reply.userRole == '2' and reply.commentParent == review.commentId}">
							<div class="reply" style="margin-left: 30px; margin-top: 10px;">
								<input type="hidden" value="${reply.commentId}" />
								<p>
									<strong>${reply.commentUserId}</strong> (${reply.commentTime})
								</p>
								<p>${reply.content}</p>
